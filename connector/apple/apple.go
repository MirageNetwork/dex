...
func (c *Config) Open(id string, logger log.Logger) (conn connector.Connector, err error) {
	...
	} else if knownBrokenAuthHeaderProvider("https://appleid.apple.com") {
		endpoint.AuthStyle = oauth2.AuthStyleInParams
	}
	...
	if len(c.Scopes) > 0 {
		...
		scopes = append(scopes, "email", "name")
	}
...
func (c *oidcConnector) HandlePOST(s connector.Scopes, r *http.Request) (identity connector.Identity, err error) {
	...
	return c.createIdentity(r.Context(), identity, token, user)
}
...
func (c *oidcConnector) createIdentity(ctx context.Context, identity connector.Identity, token *oauth2.Token, user string) (connector.Identity, error) {
	...
	if user != "" {
		...
		json.Unmarshal([]byte(user), &appleUser)
